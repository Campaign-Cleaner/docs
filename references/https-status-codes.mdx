...
6. Unofficail error responses (various)
7. Campaign Cleaner responses (`0` – `99`)

The status codes listed below are defined by [RFC 9110](https://httpwg.org/specs/rfc9110.html#overview.of.status.codes).

<Note> If you receive a response that is not in list below, it is a non-standard response, possibly custom to the server's software.</Note>
...
**428 Precondition Required**
  - The origin server requires the request to be conditional.
    This response is intended to prevent the 'lost update' problem, where a client `GET`s a resource's state, modifies it and `PUT` it back to the server, when meanwhile a third party has modified the state on the server, leading to a conflict.
...
**506 Variant Also Negotiates**
  - The server has an internal configuration errorthe chosen variant resource is configured to engage in transparent content negotiation itself, and is therefore not a proper end point in the negotiation process.
...
## Unofficial error responses

***999 Rate Limited ** 
   - This is an unofficial HTTP status response commonly used by websites to limit the number of connections within a specific time frame in order to protect the server from bots. It's often encountered on websites like LinkedIn. If you see this in your broken link report, the link is likely not broken. Instead, it means we're being rate-limited by the website. If the link works when you click it, it should also work for recipients who click it in your email or on your website. 

## Campaign Cleaner Responses
...
<Info>This information is in part provided by MDN Web Docs</Info>
