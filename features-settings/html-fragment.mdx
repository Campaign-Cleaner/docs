...
HTML fragments are sections of HTML code that can be used within an email message. These fragments offer a way for marketers to send only the necessary code for a specific part of an email, such as the header section which may include a logo, navigation, and introductory text. Fragements can be an entire email or a small snippet of code, that you will embed as HTML in a larger email template builder such as beefree.
