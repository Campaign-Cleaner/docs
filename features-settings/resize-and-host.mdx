...
description: "Optimize your email campaign performance with Campaign Cleaner's image resizing and hosting services for fast loading and a perfect display across devices."
---
...
By toggling the off switch to on and clicking `Update`. The tool will automatically resize the image, convert unsupported formats to .png, uploads the image to our CDN, and updates the campaign HTML with the new image—all in one step.
...
- Add a [Surrounding Div](/features-settings/surrounding-html-div) with a max-width.
...
## All Images Are Changed Creating a New Image Hash

Campaign Cleaner will update the hash of all images hosted on our CDN. This ensures that any images that might have been previously used in spam emails by others no longer carry a negative impact. By modifying the image, we create a unique hash, preventing spam filters from matching it to any past spamming attempts. This helps protect your reputation and ensures your emails aren’t mistake.
