...
This is a basic example, normally, there will be an external CSS those classes are attached to and you will [Inline the CSS](/classes-and-id-removal) before removing the classes.  You will also convert [`h` tags to `p` tags](/features-settings/h-to-p-tag-conversion)  to ensure a consistant display across email clients.
